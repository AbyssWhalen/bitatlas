...
经维护者 2026-09-02 明确决定，2009 题包（`apps/web/public/content/` 中的题库 JSON 与 `cn408-2009/source` 来源页图）随公开仓库发布并部署到站点，供刷题使用；原始 PDF、OCR 中间产物、`local-data/` 与学习数据仍不公开，后续年份题包公开前须逐次取得维护者授权。无题包时应用必须保持可启动，实验室、本地 PDF、设置与备份入口可用；依赖题包的流程必须明确禁用或展示空状态。显式 HTTP 缺失可进入该模式，意外网络、解析或存储错误仍须 fail closed。
...
- 原 PDF、OCR 裁图、密钥、缓存、构建产物不得提交；2009 规范化题库经维护者决定公开（2026-09-02），其余年份题包公开前须逐次授权。
