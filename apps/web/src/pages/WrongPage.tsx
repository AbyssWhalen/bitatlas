  ...
  return (
    <div className="page wrong-page">
      ...
      {wrongQuestions.length ? (
        <section className="question-list wrong-list">{wrongQuestions.map((question) => <article className="question-row" key={question.id}><span className="question-number static">{String(question.number).padStart(2, '0')}</span><div className="question-row-content"><div className="question-meta"><span>当前题面错误 {currentProgress.get(question.id)?.wrongCount ?? 0} 次</span><span>{currentProgress.get(question.id)?.mastery}</span></div><LazyContentRenderer blocks={question.stem.filter((block) => block.type !== 'image').slice(0, 1)} compact /></div><ArrowRight size={18} /></article>)}</section>
