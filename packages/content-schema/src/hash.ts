import type { ContentPack, ContentPackManifest } from '@408os/domain';

type CanonicalValue = null | boolean | number | string | CanonicalValue[] | { [key: string]: CanonicalValue };

export type ContentPackHashInput = Omit<ContentPack, 'manifest'> & {
  manifest: Omit<ContentPackManifest, 'sha256'> & { sha256?: string };
};

function assertCanonicalValue(value: unknown, path: string): asserts value is CanonicalValue {
  ...
  if (typeof value === 'object') {
    ...
    for (const [key, item] of Object.entries(value)) assertCanonicalValue(item, `${path}.${key}`);
    return;
  }
...
/** Deterministic JSON serialization: object keys are sorted and array order is retained. */
...
/**
 * Hashes the complete pack, including all manifest release fields, while excluding
 * only manifest.sha256 to avoid a recursive digest.
 */
export function computeContentPackHash(pack: ContentPackHashInput): string {
  const manifest = { ...pack.manifest };
  delete manifest.sha256;
  return sha256(canonicalSerialize({ ...pack, manifest }));
}

export function verifyContentPackHash(pack: ContentPack): boolean {
  return computeContentPackHash(pack) === pack.manifest.sha256.toLowerCase();
}
